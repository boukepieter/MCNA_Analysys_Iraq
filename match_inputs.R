...
## cluster sampling frame: make values match data
...
## replace district with camp name for idp_in_camp population group:
...
## Check if all match:
if(any(!(response$strata[response$population_group=="idp_in_camp"] %in% samplingframe_strata$stratum))){
  stop("strata not found in samplingframe")
}

if(any(is.na(response$strata))){
  stop("strata can not be NA")
}
